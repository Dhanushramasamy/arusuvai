...
const VALID_TYPES = ['veg', 'non_veg', 'premium_non_veg'];
const VALID_MEALS = ['Lunch', 'Dinner'];

export async function GET() {
  try {
    const result = await pool.query(
      `SELECT id, menu_type, day_of_week, meal_type, items, is_veg_override, updated_at
       FROM weekly_menu
       ORDER BY menu_type,
         CASE day_of_week
           WHEN 'Monday'    THEN 1
...
export async function PUT(req: NextRequest) {
  try {
    const { menu_type, day_of_week, meal_type, items, is_veg_override = false } = await req.json();
    ...
    await pool.query(
      `INSERT INTO weekly_menu (menu_type, day_of_week, meal_type, items, is_veg_override, updated_at)
       VALUES ($1, $2, $3, $4, $5, NOW())
       ON CONFLICT (menu_type, day_of_week, meal_type)
       DO UPDATE SET items = EXCLUDED.items, is_veg_override = EXCLUDED.is_veg_override, updated_at = NOW()`,
      [menu_type, day_of_week, meal_type, items, is_veg_override]
    );
